...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    #         if el >= lower_thresh and el <= upper_thresh:
    #             cp[i][j] = 0
    #         else:
    #             # print(el)
    #             cp[i][j] = 255
    img_copy = np.copy(gray_img)
    b = np.where((img_copy >= lower_thresh) & (img_copy <= upper_thresh), 1, 0)
    return b #cp
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    img_r = img_d.reshape((img_d.shape[0] * img_d.shape[1],))
...
def segment_image(img): 
    ...
    binary = threshold_segment_naive(to_grayscale(img), 0, 80)
    # edge_img = edge_detect_naive(to_grayscale(img)).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    test_img = read_image(IMG_DIR + '/test_img.jpg', grayscale=True)
    ...
    test_thresh_naive(test_img, 80, 220)
