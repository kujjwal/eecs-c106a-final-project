...
def main():
    ...
    print('Input first four joint angles for start state')
    ang1 = [float(ang) for ang in input().split(',')]
    print('Input first four joint angles for end state')
    ang2 = [float(ang) for ang in input().split(',')]
    ...
    goal.request.start_state.joint_state.position = [*ang1, 0.0, 0.0, 0.0]
    ...
    target_joint_angles = [*ang2, 0.0, 0.0, 0.0]
    tolerance = 0.0001
    consts = []
